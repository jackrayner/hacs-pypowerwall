...
- `custom_components/pypowerwall/`
  ...
  - `number.py` — `PowerwallReserveNumber`, a 0-100% slider wrapping `pw.set_reserve()`. Only two of pypowerwall's several write methods are wired up so far (this one and `select.py`'s mode control) — `set_grid_charging`/`set_grid_export` need connection-mode gating (Cloud/FleetAPI only, not TEDAPI), `schedule_max_backup`/`cancel_max_backup` need v1r-mode gating and don't map to a stateful entity, and `go_off_grid`/`reconnect_grid` physically operate the grid contactor and need explicit product sign-off before wiring up — see the write-actions discussion in project history if extending this.
  - `select.py` — `PowerwallModeSelect` wrapping `pw.set_mode()`. `BATTERY_MODES = ["self_consumption", "backup", "autonomous"]` is the exact set pypowerwall's own CLI validates writes against (`pypowerwall/__main__.py`) — not to be confused with Tesla app UI labels like "Backup-Only", which is why `strings.json`'s `select.battery_mode.state.*` keys translate the raw values into those friendlier labels for display rather than changing the values themselves.
  - Both `number.py` and `select.py` follow the same write pattern: `await hass.async_add_executor_job(coordinator.pw.<method>, value)` then `await coordinator.async_request_refresh()` so the entity's state reflects the gateway's actual response rather than assuming the write succeeded.
...
- `tests/`
  ...
  - `test_controls.py` — drives the `number`/`select` write path through the real `number.set_value`/`select.select_option` HA services (not by calling the entity method directly), then asserts both that `pw.set_reserve`/`pw.set_mode` were called with the right value *and* that the entity's displayed state updates after the mock is reconfigured to reflect the "new" gateway value — exercising the full write-then-refresh round trip, not just the write call.
