...
5. Add matching entries to **both** `custom_components/pypowerwall/strings.json` and `custom_components/pypowerwall/translations/en.json`. These two files need to stay byte-for-byte identical — `strings.json` is the actual source of truth for HA's translation system, but HACS/HA convention is for `translations/en.json` to be a literal copy of it. There's no build step that syncs them, so if you edit one and forget the other, English users will see stale or missing strings even though `strings.json` looks correct.
